...
function getLayout(user) {
  ...
  if (user?.role === 'manufacturer') return ManufacturerLayout;
  return BuyerLayout;
}

function getRoleLabel(user) {
  if (isAdmin(user))                return '🛡️ Administrator';
  if (user?.role === 'manufacturer') return '🏭 Manufacturer';
  return '🛒 Buyer';
}

function getAvatarColor(user) {
  if (isAdmin(user))                return '#27ae60';
  if (user?.role === 'manufacturer') return '#27ae60';
  return '#27ae60';
}
...
export default function ProfilePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (profileImage)     payload.append('profile_picture', profileImage);
      else if (removeImage) payload.append('profile_picture', '');
      ...
      updateUser({
        ...
        is_staff:     updatedUser.is_staff     ?? user?.is_staff,
        ...
        <div style={s.cardHeader}>
          <div style={s.avatarWrap}>
            {displayImage ? (
              ...
              <div
                style={{ ...s.avatarInitials, backgroundColor: avatarColor, cursor: editing ? 'pointer' : 'default' }}
                onClick={handleImageClick}
              >
                {initials}
              </div>
            ...
            {editing && (
              <div style={s.imageActions}>
                ...
                {(displayImage || user?.profile_picture) && !removeImage && (
          ...
          <div>
            ...
            <p style={s.cardSubtitle}>{roleLabel} · {user?.email}</p>
          </div>
        </div>
