// src/pages/manufacturer/CreateTenderPage.jsx  — MOBILE RESPONSIVE FIXED
...
export default function CreateTenderPage() {
  ...
  const [formData, setFormData] = useState({
    tender_title: '', oil_type: 'organic', quantity: '',
    tender_description: '',
    start_date: new Date().toISOString().split('T')[0],
    end_date: '', report_file: null,
  });
  ...
  const fetchNextTenderNumber = async () => {
    try { const r = await tenderAPI.getNextTenderNumber(); setTenderNumber(r.data.next_tender_number); }
    catch { setTenderNumber('TND-001'); }
  };
  ...
  const validateImageFile = (file) => new Promise((resolve, reject) => {
    const reader = new FileReader();
    reader.onload = (e) => { const img = new Image(); img.onload = () => resolve(true); img.onerror = () => reject(new Error('Invalid image')); img.src = e.target.result; };
  ...
  const handleFileChange = async (e) => {
    ...
    setQualityGrade(''); setQualityScore('');
    const ext = file.name.split('.').pop().toLowerCase();
    if (!['pdf','doc','docx','xls','xlsx','png','jpg','jpeg'].includes(ext)) { setErrors(prev => ({ ...prev, report_file: 'Invalid file type. Allowed: PDF, DOC, XLS, PNG, JPG' })); return; }
    if (file.size > 10 * 1024 * 1024) { setErrors(prev => ({ ...prev, report_file: 'File size cannot exceed 10MB' })); return; }
    if (['jpg','jpeg','png'].includes(ext)) { try { await validateImageFile(file); } catch { setErrors(prev => ({ ...prev, report_file: 'Invalid or corrupted image file' })); return; } }
    setFormData(prev => ({ ...prev, report_file: file }));
    try { await analyzeQuality(file); } catch {}
  };

  const analyzeQuality = async (file) => {
    setReportAnalyzing(true); setQualityGrade(''); setQualityScore('');
    const fd = new FormData(); fd.append('file', file);
    try {
      const r = await tenderAPI.predictQuality(fd);
      if (r.data?.quality_grade) { setQualityGrade(r.data.quality_grade); setQualityScore(r.data.quality_score); }
    } catch (err) {
    ...
    } finally { setReportAnalyzing(false); }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response?.data) {
        const mapped = {};
        Object.keys(err.response.data).forEach(k => { mapped[k] = Array.isArray(err.response.data[k]) ? err.response.data[k][0] : err.response.data[k]; });
        setErrors(mapped); setServerError('Please fix the errors above.');
      } else { setServerError('Failed to create tender. Please try again.'); }
    } finally { setLoading(false); }
  };

  /* On mobile, side-by-side rows collapse to single column */
  const rowStyle = isMobile ? { display:'flex', flexDirection:'column', gap:0 } : s.row;
  ...
  return (
    <ManufacturerLayout>
      ...
      <div style={{ ...s.card, maxWidth: isMobile ? '100%' : 780 }}>
        <form onSubmit={handleSubmit}>
          ...
          <div style={s.formGroup}>
            ...
            <input type="text" name="tender_title" value={formData.tender_title} onChange={handleChange} placeholder="e.g., Premium Cinnamon Oil Supply"
          ...
          <div style={rowStyle}>
            ...
            <div style={s.formGroup}>
              ...
              <input type="number" name="quantity" value={formData.quantity} onChange={handleChange} placeholder="e.g., 500" step="0.01" min="0"
          ...
          <div style={s.formGroup}>
            ...
            <div style={s.fileBox}>
              <input type="file" id="report_file" onChange={handleFileChange} accept=".pdf,.doc,.docx,.xls,.xlsx,.png,.jpg,.jpeg" style={{ display:'none' }} />
            ...
            {reportAnalyzing && <div style={s.analyzing}>🔍 Analyzing report quality…</div>}
          </div>
          ...
          <div style={s.formGroup}>
            ...
            <textarea name="tender_description" value={formData.tender_description} onChange={handleChange} rows={4}
          ...
          <div style={{ ...s.formActions, flexDirection: isMobile ? 'column' : 'row' }}>
            <button type="button" style={{ ...s.cancelBtn, width: isMobile ? '100%' : 'auto' }} onClick={() => navigate('/manufacturer/my-tenders')}>Cancel</button>
            <button type="submit" disabled={loading || reportAnalyzing}
              style={{ ...s.submitBtn, ...(loading || reportAnalyzing ? s.submitBtnDisabled : {}), width: isMobile ? '100%' : 'auto' }}>
...
const s = {
  ...
  backBtn:       { padding:'0.5rem 1rem', backgroundColor:'#f0f2f5', color:'#1a2e44', border:'1px solid #e2e8f0', borderRadius:7, cursor:'pointer', fontSize:'0.875rem', fontWeight:500, flexShrink:0 },
  card:          { backgroundColor:'#fff', borderRadius:12, boxShadow:'0 2px 12px rgba(0,0,0,0.08)', padding:'1.5rem' },
  ...
  formGroup:     { marginBottom:'1.1rem', flex:1 },
  ...
  fileLabel:     { display:'inline-flex', alignItems:'center', gap:'0.5rem', padding:'0.65rem 1rem', border:'1.5px dashed #cbd5e1', borderRadius:7, cursor:'pointer', fontSize:'0.875rem', color:'#475569', backgroundColor:'#f8fafc', maxWidth:'100%', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' },
  ...
  qualityResult: { backgroundColor:'#f0fdf4', padding:'1rem 1.1rem', borderRadius:8, marginBottom:'1.1rem', border:'2px solid #86efac' },
  qualityBadge:  { display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'0.35rem', color:'#15803d', fontSize:'1rem', fontWeight:700, flexWrap:'wrap', gap:'0.5rem' },
  qualityScore:  { fontSize:'0.875rem', color:'#64748b', fontWeight:400 },
  aiLabel:       { fontSize:'0.78rem', color:'#64748b', fontStyle:'italic' },
  formActions:   { display:'flex', justifyContent:'flex-end', gap:'0.75rem', marginTop:'0.5rem', paddingTop:'1.1rem', borderTop:'1px solid #e2e8f0' },
