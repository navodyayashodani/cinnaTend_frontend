// src/pages/manufacturer/AnalyticsPage.jsx  — MOBILE RESPONSIVE FIXED
...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    tenderAPI.getAllTenders().then(r => setAllTenders(r.data)).catch(console.error).finally(() => setLoading(false));
  }, []);
  ...
  const gradeCount = myTenders.reduce((acc, t) => { if (t.quality_grade) acc[t.quality_grade] = (acc[t.quality_grade]||0) + 1; return acc; }, {});
    ...
    <ManufacturerLayout>
      ...
      {loading ? (
        ...
        <>
          ...
          {/* Status + Grade breakdown — stack on mobile */}
          <div style={{ ...s.row, gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr' }}>
            <div style={s.panel}>
              <p style={s.panelTitle}>Tender Status Breakdown</p>
              {[
                { label:'Active',    count:active.length,    color:'#16a34a' },
                { label:'Expired',   count:expired.length,   color:'#b91c1c' },
                { label:'Completed', count:completed.length, color:'#d4922a' },
              ].map(item => {
                ...
                return (
                  <div key={item.label} style={s.barRow}>
                    ...
                    <div style={s.barTrack}><div style={{ ...s.barFill, width:`${pct}%`, backgroundColor:item.color }} /></div>
              ...
              {Object.keys(gradeCount).length === 0 ? (
                ...
                return (
                  <div key={g} style={s.barRow}>
                    ...
                    <div style={s.barTrack}><div style={{ ...s.barFill, width:`${pct}%`, backgroundColor:gColors[g] }} /></div>
              ...
              {myTenders.slice(0, 10).map(t => {
                ...
                return (
                  <div key={t.id} style={s.bidBarItem}>
                    <span style={{ ...s.bidBarLabel, minWidth: isMobile ? 100 : 140 }}>{t.tender_title?.length > maxLen ? t.tender_title.slice(0, maxLen)+'…' : t.tender_title}</span>
                    <div style={s.bidBarTrack}><div style={{ ...s.bidBarFill, width:`${pct}%`, backgroundColor:color }} /></div>
                    <span style={{ ...s.bidBarCount, color }}>{t.bid_count||0}</span>
                  </div>
                );
              })}
...
const s = {
  ...
  kpiGrid:      { display:'grid', gap:'1rem', marginBottom:'1.5rem' },
  kpiCard:      { backgroundColor:'#fff', border:'1.5px solid #e2e8f0', borderRadius:10, padding:'1.1rem', display:'flex', flexDirection:'column', alignItems:'center', gap:'0.3rem', textAlign:'center' },
  kpiIcon:      { fontSize:'1.4rem' },
  kpiVal:       { fontSize:'1.6rem', fontWeight:800, lineHeight:1.1 },
  kpiLabel:     { fontSize:'0.72rem', color:'#64748b', fontWeight:500 },
  row:          { display:'grid', gap:'1.25rem', marginBottom:'1.25rem' },
  panel:        { backgroundColor:'#fff', border:'1.5px solid #e2e8f0', borderRadius:10, padding:'1.4rem', marginBottom:'1.25rem' },
  panelTitle:   { margin:'0 0 1.1rem', color:'#1a2e44', fontWeight:700, fontSize:'1rem' },
  barRow:       { display:'flex', alignItems:'center', gap:'0.6rem', marginBottom:'0.8rem' },
  barLabel:     { minWidth:75, fontSize:'0.82rem', color:'#4a5568', fontWeight:500 },
  ...
  barCount:     { minWidth:70, fontSize:'0.78rem', fontWeight:600, textAlign:'right' },
  noData:       { color:'#94a3b8', fontSize:'0.875rem', margin:0 },
  bidBars:      { display:'flex', flexDirection:'column', gap:'0.65rem' },
  bidBarItem:   { display:'flex', alignItems:'center', gap:'0.6rem' },
  bidBarLabel:  { fontSize:'0.8rem', color:'#4a5568', fontWeight:500, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' },
  bidBarTrack:  { flex:1, height:12, backgroundColor:'#f1f5f9', borderRadius:5, overflow:'hidden' },
  bidBarFill:   { height:'100%', borderRadius:5, transition:'width 0.4s ease' },
  bidBarCount:  { minWidth:40, fontSize:'0.78rem', fontWeight:600, textAlign:'right' },
};